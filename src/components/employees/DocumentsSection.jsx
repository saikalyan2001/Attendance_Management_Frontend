// components/shared/DocumentsSection.jsx
...
import { Upload, Loader2, Eye, Search, FileText, Image as ImageIcon, User, ChevronUp, ChevronDown, FilePlus, Trash2, ChevronLeft, ChevronRight, ArrowUpDown } from 'lucide-react';
...
const DocumentsSection = ({
  // Data props
  documents = [],
  documentsPagination = { totalPages: 1, page: 1 },
  employeeName = 'Documents',
  employeeId,
  ...
  const handleSearchSubmit = () => {
    if (onSearchDocuments) {
      onSearchDocuments(localSearchQuery);
    }
    if (setSearchQuery) {
  ...
  const handleDocumentSubmit = async (data) => {
    ...
    try {
      ...
      await onUploadDocuments(data.documents);
      ...
      >
        <CardHeader className="p-2 xs:p-3 sm:p-6">
          <CardTitle id="employee-documents-title" className="text-base xs:text-lg sm:text-xl md:text-2xl font-semibold flex items-center gap-1 xs:gap-2 sm:gap-3">
            ...
            <span className="truncate">{title || `${employeeName ? `${employeeName}'s Documents` : 'Documents'}`}</span>
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="p-2 xs:p-3 sm:p-6 space-y-4 xs:space-y-5 sm:space-y-6">
          ...
          <Collapsible open={isTableOpen} onOpenChange={setIsTableOpen}>
            ...
            <CollapsibleContent className="mt-2 xs:mt-3 sm:mt-4">
                ...
                <Table className="w-full" role="grid">
                  <TableHeader>
                    <TableRow className="bg-accent/10 rounded-lg">
                      <TableHead className="text-xs xs:text-sm sm:text-base font-semibold px-2 xs:px-3 sm:px-4 py-2 xs:py-3 min-w-[120px] max-w-[150px] text-left break-words">
                        {showSorting ? (
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      <TableRow>
                        ...
                        >
                          <div className="flex flex-col items-center gap-2">
                            <p>{localSearchQuery ? 'No documents match your search.' : 'No documents available. Please upload documents.'}</p>
                            {showUpload && !localSearchQuery && (
                              <Button
                                variant="outline"
                                onClick={openUploadDialog}
                                className="border-accent text-accent hover:bg-accent-hover hover:text-body rounded-lg px-2 xs:px-3 sm:px-4 py-1 xs:py-2 text-xs xs:text-sm sm:text-base transition-all duration-300 focus:ring-2 focus:ring-accent focus:ring-offset-2 min-h-[36px]"
                      ...
                      processedDocuments.map((doc) => (
                        <TableRow key={doc._id || doc.id} className="hover:bg-accent/5 transition-colors">
              ...
              {documentsPagination.totalPages > 1 && !isLoading && (
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="outline"
                          onClick={() => handlePageChange(currentPage + 1)}
                          disabled={currentPage === documentsPagination.totalPages || isLoading}
                          className="border-accent text-accent hover:bg-accent-hover hover:text-body rounded-lg px-4 py-2 text-xs xs:text-sm sm:text-base transition-all duration-300 focus:ring-2 focus:ring-accent focus:ring-offset-2 min-h-[36px]"
            ...
            <form className="space-y-3 xs:space-y-4 sm:space-y-5 p-2 xs:p-3 sm:p-4" ref={formRef}>
                                  ...
                                  <div className="flex gap-1 xs:gap-2">
                                    <Button
                                      ...
                                      className={cn(
                                        ...
                                        (isSubmitting || isLoading || !previewUrls[index]) && 'opacity-50 cursor-not-allowed'
                                      )}
                                      aria-label={`Preview document ${formField.value.name}`}
                                      disabled={isSubmitting || isLoading || !previewUrls[index]}
                                    >
              ...
              <div className="flex justify-between items-center mt-2 xs:mt-3 sm:mt-4">
                <Button
                  ...
                  className="bg-accent text-body hover:bg-accent-hover rounded-md text-xs xs:text-sm sm:text-base py-1 xs:py-1.5 sm:py-2 px-2 xs:px-3 sm:px-4 transition-all duration-300 hover:shadow-md min-h-[32px] xs:min-h-[36px]"
                  disabled={documentFields.length >= 5 || isSubmitting || isLoading}
