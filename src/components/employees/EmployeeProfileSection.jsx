...
const EmployeeProfileSection = ({
  ...
  // Debug form state when modal opens
  useEffect(() => {
    if (editDialogOpen) {
    }
  }, [editDialogOpen, editForm]);

  // Total yearly calculation for display reference
  const totalYearlyPaidLeaves = useMemo(() => {
    if (!employee?.joinDate || !settings) {
      return settings?.paidLeavesPerYear || 24;
    }

    let paidLeavesPerYear = settings.paidLeavesPerYear || 24;

    if (
      settings.locationLeaveSettings &&
      settings.locationLeaveSettings.length > 0 &&
      employee.location
    ) {
    ...
    if (joinYear === currentYear) {
      ...
      const proRatedLeaves = Math.round(
        (paidLeavesPerYear * remainingMonths) / 12
      );
  ...
  // Display capped leave usage from actual Redux state
  const currentAvailableLeaves = useMemo(() => {
    return employee?.paidLeaves?.available || 0;
  }, [employee?.paidLeaves?.available]);

  // Calculate display usage with proper formatting
  const displayUsedLeaves = useMemo(() => {
    const usedValue = employee?.paidLeaves?.used || 0;
    return usedValue % 1 === 0 ? usedValue.toString() : usedValue.toFixed(1);
  }, [employee?.paidLeaves?.used]);
  ...
  }, [employee?.paidLeaves?.available, employee?.paidLeaves?.used]);
  ...
  const handleEditSubmit = async (data) => {
    try {
      ...
      await onEditClick(employee._id, employeeData);
    ...
    try {
      ...
      if (!isValid) {
        ...
        for (const field of fieldOrder) {
          const error =
            editForm.formState.errors[field.split(".")[0]]?.[
              field.split(".")[1]
            ] || editForm.formState.errors[field];
          if (error?.message) {
            const fieldLabel = fieldLabels[field] || field;
            const displayMessage = error.message.includes("is required")
        ...
        if (
          editForm.formState.errors.bankDetails &&
          editForm.formState.errors.bankDetails.message
        ) {
          errors.push({
            field: "bankDetails",
            message: editForm.formState.errors.bankDetails.message,
          });
        }
  ...
  const openEditDialog = (emp) => {
    ...
    try {
      const safeEmp = emp || {};
      const safePaidLeaves = safeEmp.paidLeaves || {
        available: 0,
        used: 0,
        carriedForward: 0,
      };
      const safeBankDetails = safeEmp.bankDetails || {
        accountNo: "",
        ifscCode: "",
        bankName: "",
        accountHolder: "",
      ...
      editForm.reset({
        name: safeEmp.name || "",
        email: safeEmp.email || "",
        designation: safeEmp.designation || "",
        department: safeEmp.department || "",
        salary: safeEmp.salary ? safeEmp.salary.toString() : "",
        phone: safeEmp.phone || "",
        dob: safeEmp.dob ? new Date(safeEmp.dob).toISOString().split("T")[0] : "",
        bankDetails: {
          accountNo: safeBankDetails.accountNo || "",
          ifscCode: safeBankDetails.ifscCode || "",
          bankName: safeBankDetails.bankName || "",
          accountHolder: safeBankDetails.accountHolder || "",
        },
        paidLeaves: {
          available: safePaidLeaves.available
            ? safePaidLeaves.available.toString()
            : "0",
          used: safePaidLeaves.used ? safePaidLeaves.used.toString() : "0",
          carriedForward: safePaidLeaves.carriedForward
            ? safePaidLeaves.carriedForward.toString()
            : "0",
        },
      });
      setEditDialogOpen(true);
    } catch (error) {
      toast.error("Failed to initialize edit form", {
        id: "form-init-error",
        duration: 5000,
        position: "top-center",
      });
      ...
      >
        <CardHeader className="flex flex-row flex-wrap items-center justify-between px-2 xs:px-3 sm:px-6 py-2 xs:py-3 sm:py-6 gap-1 xs:gap-2">
          <CardTitle
            ...
            className="text-2xs xs:text-base sm:text-2xl md:text-3xl font-bold flex items-center gap-1 xs:gap-2 sm:gap-3 truncate"
          >
          ...
          {canEdit && (
            <Button
              ...
              aria-label={`Edit ${title.toLowerCase()} for ${employee.name}`}
            >
        ...
        <CardContent className="px-2 xs:px-3 sm:px-6 py-2 xs:py-3 sm:py-6 space-y-3 xs:space-y-4 sm:space-y-6">
          <Collapsible open={isPersonalOpen} onOpenChange={setIsPersonalOpen}>
            <CollapsibleTrigger className="flex items-center justify-between w-full text-2xs xs:text-xs sm:text-lg md:text-xl font-semibold text-body hover:text-accent transition-colors">
              Personal Details
              {isPersonalOpen ? (
                <ChevronUp className="h-2.5 w-2.5 xs:h-3 xs:w-3 sm:h-5 sm:w-5" />
              ) : (
                <ChevronDown className="h-2.5 w-2.5 xs:h-3 xs:w-3 sm:h-5 sm:w-5" />
              )}
            </CollapsibleTrigger>
            <CollapsibleContent className="mt-1 xs:mt-1.5 sm:mt-4 animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-4 sm:gap-6">
                <div>
                  <Label className="text-2xs xs:text-xs sm:text-sm md:text-base font-semibold text-body">
                    Name
                  </Label>
                  <div className="flex items-center mt-0.5 xs:mt-1 sm:mt-2 group">
                    <p className="text-body text-xs xs:text-sm sm:text-base md:text-lg truncate">
                      {employee.name}
                    </p>
                    <TooltipProvider delayDuration={200}>
                      <Tooltip>
                        ...
                        <TooltipContent
                          className="bg-complementary text-body border-accent text-2xs xs:text-xs sm:text-base max-w-[75vw] xs:max-w-[85vw] sm:max-w-none z-[100]"
                          side="top"
                        >
                          Copy name
                        </TooltipContent>
                ...
                <div>
                  ...
                  <div className="flex items-center mt-0.5 xs:mt-1 sm:mt-2 group">
                    <p className="text-body text-xs xs:text-sm sm:text-base md:text-lg">
                      {employee.employeeId}
                    </p>
                    <TooltipProvider delayDuration={200}>
                      <Tooltip>
                  ...
                  <div className="flex items-center mt-0.5 xs:mt-1 sm:mt-2 group">
                    ...
                    {employee.phone && (
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <CopyButton text={employee.phone} fieldId="phone" />
                          </TooltipTrigger>
                          <TooltipContent
                            className="bg-complementary text-body border-accent text-2xs xs:text-xs sm:text-base max-w-[75vw] xs:max-w-[85vw] sm:max-w-none z-[100]"
                            side="top"
              ...
              <CollapsibleContent className="mt-1 xs:mt-1.5 sm:mt-4 animate-fade-in">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 xs:gap-4 sm:gap-6">
                  <div>
                    ...
                    <div className="flex items-center mt-0.5 xs:mt-1 sm:mt-2 group">
                      <p className="text-body text-xs xs:text-sm sm:text-base md:text-lg">
                        {employee.bankDetails.accountNo || "N/A"}
                      </p>
                      {employee.bankDetails.accountNo && (
                        <TooltipProvider delayDuration={200}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <CopyButton
                                text={employee.bankDetails.accountNo}
                                fieldId="accountNo"
                              />
                            </TooltipTrigger>
                            <TooltipContent
                      ...
                      {employee.bankDetails.ifscCode && (
                        <TooltipProvider delayDuration={200}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <CopyButton
                                text={employee.bankDetails.ifscCode}
                                fieldId="ifscCode"
                              />
                            </TooltipTrigger>
                            <TooltipContent
                      ...
                      {employee.bankDetails.bankName && (
                        <TooltipProvider delayDuration={200}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <CopyButton
                                text={employee.bankDetails.bankName}
                                fieldId="bankName"
                              />
                            </TooltipTrigger>
                            <TooltipContent
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-0.5 xs:mt-1 sm:mt-2 group">
                      <p className="text-body text-xs xs:text-sm sm:text-base md:text-lg">
                        {employee.bankDetails.accountHolder || "N/A"}
                      </p>
                      {employee.bankDetails.accountHolder && (
                        <TooltipProvider delayDuration={200}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <CopyButton
                                text={employee.bankDetails.accountHolder}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        {serverError?.fields?.name && (
                          <p className="text-error text-2xs xs:text-xs sm:text-sm">
                            {serverError.fields.name}
                          </p>
                        )}
                      </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormMessage className="text-error text-2xs xs:text-xs sm:text-sm" />
                          {serverError?.fields?.["paidLeaves.available"] && (
                            <p className="text-error text-2xs xs:text-xs sm:text-sm">
                              {serverError.fields["paidLeaves.available"]}
                            </p>
                          )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormMessage className="text-error text-2xs xs:text-xs sm:text-sm" />
                          {serverError?.fields?.["paidLeaves.used"] && (
                            <p className="text-error text-2xs xs:text-xs sm:text-sm">
                              {serverError.fields["paidLeaves.used"]}
                            </p>
                          )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormMessage className="text-error text-2xs xs:text-xs sm:text-sm" />
                          {serverError?.fields?.["paidLeaves.carriedForward"] && (
                            <p className="text-error text-2xs xs:text-xs sm:text-sm">
                              {serverError.fields["paidLeaves.carriedForward"]}
                            </p>
                          )}
