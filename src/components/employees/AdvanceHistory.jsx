...
const AdvanceHistory = ({
  ...
  id,
  itemsPerPage = 5,
  role, // 'superadmin', 'admin', or 'siteincharge'
}) => {
  const [isTableOpen, setIsTableOpen] = useState(true);
  const autoDismissDuration = 5000;

  // For siteincharge, use local sorting since data is already fetched
  const sortedAdvances = useMemo(() => {
    if (role === 'siteincharge' && advances && Array.isArray(advances)) {
  ...
  // Determine which advances to display based on role
  const displayAdvances = role === 'siteincharge' ? sortedAdvances : advances;

  // For superadmin/admin, use Redux state for error handling
            ...
            <div className="overflow-x-auto">
              <Table className="w-full" role="grid">
                ...
                <TableBody>
                  {isLoading ? (
                  ...
                  ) : error && (role === 'superadmin' || role === 'admin') ? (
                    <TableRow>
                      <TableCell
                        colSpan={3}
                    ...
                    displayAdvances.map((advance, index) => (
