// components/shared/SalaryTable.jsx
...
const SalaryTable = ({
  ...
  config = {
    ...
    toastConfig: {
      id: "export-error", // For sonner compatibility
      duration: 6000,
      position: "top-center",
    },
    useBackendSummary: false, // SuperAdmin uses backend summary, Admin calculates
  }
}) => {
  // Calculate totals for Admin version
  const calculateTotals = (data) => {
    ...
    return data.reduce(
      (acc, emp) => ({
        ...
        presentDays: acc.presentDays + (typeof emp.presentDays === "number" ? emp.presentDays : 0),
        halfDays: acc.halfDays + (typeof emp.halfDays === "number" ? emp.halfDays : 0),
        absentDays: acc.absentDays + (typeof emp.absentDays === "number" ? emp.absentDays : 0),
        leaveDays: acc.leaveDays + (typeof emp.leaveDays === "number" ? emp.leaveDays : 0),
  ...
  // Use backend summary for SuperAdmin, calculate for Admin
  const totals = useMemo(() => {
    if (config.useBackendSummary && salaryReport?.summary) {
      return {
        employeeCount: salaryReport.employees?.length || 0,
        uniqueLocations: new Set(
          (salaryReport.employees || []).map((emp) => emp.location?.name || "Unknown")
        ).size,
        presentDays: salaryReport.summary.totalPresentDays || 0,
        halfDays: salaryReport.summary.totalHalfDays || 0,
        absentDays: salaryReport.summary.totalAbsentDays || 0,
        leaveDays: salaryReport.summary.totalLeaveDays || 0,
        grossSalary: salaryReport.summary.totalGrossSalary || 0,
        netSalary: salaryReport.summary.totalNetSalary || 0,
        advance: salaryReport.summary.totalAdvance || 0,
        totalSalary: salaryReport.summary.totalSalary || 0,
      };
    } else {
      return calculateTotals(salaryReport?.employees || []);
    }
  }, [salaryReport, config.useBackendSummary]);

  const handleDownloadPDF = () => {
    if (!salaryReport?.employees?.length) {
      const toastOptions = config.isSuperAdmin 
        ? { duration: config.toastConfig.duration, position: config.toastConfig.position }
        : { 
            id: config.toastConfig.id, 
            duration: config.toastConfig.duration, 
            position: config.toastConfig.position 
          };
      
      config.showToast?.error("No salary data to export", toastOptions);
      return;
    }
    ...
    const filename = config.isSuperAdmin 
      ? `superadmin-salary-report-${month}-${year}.pdf`
      : `salary-report-${month}-${year}.pdf`;
  ...
  const handleDownloadExcel = () => {
    ...
    const filename = config.isSuperAdmin 
      ? `superadmin-salary-report-${month}-${year}.xlsx`
      : `salary-report-${month}-${year}.xlsx`;
  ...
  // SuperAdmin uses sortedSalary (filtered and sorted), Admin uses direct data
  const displayData = config.isSuperAdmin ? sortedSalary : (salaryReport?.employees || []);
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="mb-4">
          <p className="text-sm md:text-base text-body/80">
            Total Employees: {totals.employeeCount} | Total Locations: {totals.uniqueLocations} | 
            Total Present Days: {totals.presentDays} | Total Half Days: {totals.halfDays} | 
            Total Absent Days: {totals.absentDays} | Total Leave Days: {totals.leaveDays} | 
            Total Gross Salary: ₹{formatter.format(totals.grossSalary)} | 
            Total Net Salary: ₹{formatter.format(totals.netSalary)} | 
            Total Advance: ₹{formatter.format(totals.advance)} | 
                    ...
                    <TooltipProvider key={page}>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            variant={
                              currentPage.salary === page ? "default" : "outline"
                            }
                            onClick={() =>
                              setCurrentPage({
                                ...currentPage,
