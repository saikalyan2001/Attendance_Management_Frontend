// components/shared/LeaveTable.jsx
...
const LeaveTable = ({
  ...
  const handleDownloadPDF = () => {
    if (!leaveReport?.employees?.length) {
      ...
      config.showToast?.error("No leave data to export", toastOptions);
      return;
    }
    ...
    const filename = config.isSuperAdmin 
      ? `superadmin-leave-report-${month}-${year}.pdf`
      : `leave-report-${month}-${year}.pdf`;
  ...
  const handleDownloadExcel = () => {
    ...
    const filename = config.isSuperAdmin 
      ? `superadmin-leave-report-${month}-${year}.xlsx`
      : `leave-report-${month}-${year}.xlsx`;
  ...
  // SuperAdmin uses sortedLeave (filtered and sorted), Admin uses direct data
  const displayData = config.isSuperAdmin ? sortedLeave : (leaveReport?.employees || []);
