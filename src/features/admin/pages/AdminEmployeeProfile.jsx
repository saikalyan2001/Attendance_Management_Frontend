...
const AdminEmployeeProfile = () => {
  ...
  // Fetch employee data on mount and when id changes
  useEffect(() => {
    if (user?.role !== 'admin') {
      navigate('/login');
      return;
    ...
    const fetchData = async () => {
      try {
        ...
        if (!result) {
          toast.error("Employee not found", { id: 'employee-not-found', duration: 5000, position: 'top-center' });
          navigate('/admin/employees');
        }
      } catch (err) {
        toast.error("Failed to fetch employee data", { id: 'fetch-employee-error', duration: 5000, position: 'top-center' });
        navigate('/admin/employees');
      }
    };
  ...
  }, [dispatch, id, activeTab, monthFilter, yearFilter, currentPage, sortField, sortOrder]);
  ...
  }, [dispatch, id, activeTab, advancesCurrentPage, advancesSortField, advancesSortOrder]);

  // Fetch documents when tab is active and parameters change
  useEffect(() => {
  ...
  }, [dispatch, id, activeTab, documentsCurrentPage, documentsSearchQuery]);
        ...
        <div className="mt-4">
          {activeTab === 'profile' && currentEmployee && (
            <EmployeeProfileSection
              ...
              onEditClick={handleEditSubmit} // Fixed: Use openEditDialog instead of handleEditSubmit
              title="Profile"
              canEdit={true}
              settings={settings}
            />
          ...
          {activeTab === 'advances' && (
            <AdvanceHistory
              advances={advances}
              advancesPagination={advancesPagination}
              currentPage={advancesCurrentPage}
              setCurrentPage={setAdvancesCurrentPageCallback}
              sortField={advancesSortField}
              setSortField={setAdvancesSortFieldCallback}
              sortOrder={advancesSortOrder}
              setSortOrder={setAdvancesSortOrderCallback}
              employeeName={currentEmployee.name}
              isLoading={loading}
              dispatch={dispatch}
              id={id}
              ...
              role="admin"
              // Removed fetchEmployeeAdvances prop since fetching is handled in parent
            />
          )}
          {activeTab === 'documents' && (
  <DocumentsSection
