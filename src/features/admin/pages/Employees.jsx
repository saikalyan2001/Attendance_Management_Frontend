...
import { fetchEmployees, updateEmployee, deactivateEmployee, transferEmployee, rejoinEmployee, addEmployeeDocuments, reset as resetEmployees, fetchSettings } from '../redux/employeeSlice';
...
import { Loader2, Trash2, Search, ArrowUpDown, AlertCircle, X, CheckCircle, PlusCircle, Users, ChevronLeft, ChevronRight, FilePlus, Truck, Eye, FileText, Image as ImageIcon, History, UserPlus, LogOut } from 'lucide-react';
...
const Employees = () => {
  ...
  const { employees, settings, loading: employeesLoading, error: employeesError, success } = useSelector((state) => state.adminEmployees);
  ...
  useEffect(() => {
    if (employeesError || locationsError) {
      setErrorMessage(employeesError || locationsError);
  ...
  dispatch(rejoinEmployee({ id: employeeToRejoin._id, rejoinDate: new Date(data.rejoinDate).toISOString() }))
    ...
    .catch((error) => {
      // Extract the error message if error is an object, otherwise use the default message
      const errorMessage = typeof error === 'string' ? error : error?.message || 'Failed to rejoin employee';
      toast.error(errorMessage, { duration: autoDismissDuration });
    });
};
  ...
  const handleAddDocumentsSubmit = (data) => {
    dispatch(addEmployeeDocuments({ id: addDocumentsEmployeeId, documents: data.documents })).then((result) => {
      if (result.meta.requestStatus === 'fulfilled') {
  ...
  return (
    <Layout title="Employees">
      {(errorMessage || employeesError || locationsError) && showErrorAlert && (
        <Alert
          variant="destructive"
          className={cn(
        ...
        >
          ...
          <AlertDescription className="text-[10px] sm:text-sm md:text-base xl:text-lg">
            {errorMessage || employeesError || locationsError}
          </AlertDescription>
          <Button
            variant="ghost"
            size="sm"
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center w-full sm:w-auto">
              ...
              <Select value={filterLocation} onValueChange={handleFilterLocationChange}>
                ...
                <SelectContent className="bg-complementary text-body">
                  <SelectItem value="all" className="text-[10px] sm:text-sm xl:text-base">All Locations</SelectItem>
                  {locations.map((loc) => (
      ...
      <Dialog open={openEditDialog} onOpenChange={setOpenEditDialog}>
        <DialogContent className="bg-complementary text-body max-h-[80vh] overflow-y-auto max-w-[90vw] sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg md:text-xl xl:text-2xl font-bold">Edit Employee: {editEmployee?.name}</DialogTitle>
          </DialogHeader>
          <Form {...updateForm}>
            <form onSubmit={updateForm.handleSubmit(handleEditSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
              <FormItem>
              ...
              <FormItem>
                ...
                <Input
                  value={editEmployee?.location?.name || editEmployee?.location?.city || 'N/A'}
                  className="h-9 sm:h-10 xl:h-12 bg-body text-body border-complementary focus:border-accent focus:ring-2 focus:ring-accent/20 rounded-md text-[10px] sm:text-sm xl:text-lg transition-all duration-300 hover:shadow-sm"
                  disabled
                  aria-label="Location"
                />
        ...
        <DialogContent className="bg-complementary text-body max-w-[90vw] sm:max-w-md">
          ...
          <DialogFooter className="mt-4">
            ...
            <Button
              ...
              className="bg-error text-body hover:bg-error-hover rounded-md text-[10px] sm:text-sm xl:text-lg py-1 sm:py-2 px-3 sm:px-4 min-h-[40px] sm:min-h-[48px] transition-all duration-300 hover:shadow-md hover:scale-105 animate-pulse"
              disabled={employeesLoading}
