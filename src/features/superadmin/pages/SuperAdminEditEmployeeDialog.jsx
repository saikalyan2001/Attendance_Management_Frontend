...
import { updateEmployee } from '../redux/superadminEmployeeSlice'; // Superadmin thunk

// Validation schema for superadmin
const updateEmployeeSchema = z.object({
  ...
  paidLeaves: z
    .object({
      available: z.number().min(0, 'Available leaves cannot be negative').max(24, 'Available leaves cannot exceed 24'),
...
const SuperAdminEditEmployeeDialog = ({ open, onOpenChange, employee, setSuccessMessage }) => {
    ...
    try {
      ...
      if (!isValid) {
        ...
        const fieldOrder = [
          ...
          'phone',
          'dob',
          'bankDetails', // Check refinement error here
        ...
        if (errors.length > 0) {
          ...
          const firstErrorField = firstError.field.includes('.')
            ...
            : firstError.field === 'bankDetails'
            ? document.querySelector('[name="bankDetails.accountNo"]') || formRef.current
