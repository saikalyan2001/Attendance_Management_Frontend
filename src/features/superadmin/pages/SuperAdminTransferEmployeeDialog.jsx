...
import { fetchEmployeeById, transferEmployee, reset as resetEmployees } from '../redux/superadminEmployeeSlice';
...
const SuperAdminTransferEmployeeDialog = ({ open, onOpenChange, employeeId, setSuccessMessage }) => {
  ...
  const { currentEmployee, loading: employeesLoading, error } = useSelector((state) => state.superadminEmployees);
        ...
        <Form {...form}>
          <form className="space-y-4" ref={formRef}>
            <div className="text-sm">
              ...
              <p><strong>Current Location:</strong> {currentEmployee.location?.name || currentEmployee.location?.city || 'N/A'}</p>
            </div>
            <FormField
