...
const SuperAdminAttendanceTable = ({
  attendanceReport,
  locations,
  reportsLoading,
  month,
  ...
  const hasMissingEmployeeData = useMemo(
    () => attendanceReport?.attendance?.some((record) => !record.employee),
    [attendanceReport]
  );
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text("Superadmin Attendance Report", 14, 15);
    doc.setFontSize(10);
    doc.text(
    ...
    doc.save(`superadmin-attendance-report-${month}-${year}.pdf`);
  };
  ...
  const handleDownloadExcel = () => {
    ...
    const headerWs = XLSX.utils.json_to_sheet(
      [
        { A: "Superadmin Attendance Report" },
        {
    ...
    const filename = `superadmin-attendance-report-${month}-${year}.xlsx`;
    ...
    <Card className="bg-complementary text-body max-w-7xl mx-auto shadow-xl rounded-xl border border-accent/20 animate-fade-in mb-6">
      <CardHeader className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <span className="text-xl md:text-2xl font-bold">Superadmin Attendance Report</span>
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <Button
              ...
              aria-label="Download superadmin attendance report as Excel"
            ...
            <Button
              ...
              className="bg-accent text-body hover:bg-accent-hover rounded-lg text-sm py-2 px-4 transition-all duration-300 hover:shadow-md flex items-center gap-2"
              disabled={reportsLoading || !attendanceReport?.attendance?.length}
              aria-label="Download superadmin attendance report as PDF"
