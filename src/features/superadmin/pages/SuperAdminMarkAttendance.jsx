...
import { fetchEmployees, setEmployees } from '../redux/superadminEmployeeSlice';
...
import { CalendarIcon, Loader2, CheckCircle2, Users, Search, Eye, ChevronLeft, ChevronRight, X } from 'lucide-react';
import { Alert, AlertDescription } from '@/components/ui/alert';
import toast from 'react-hot-toast';

const SuperAdminMarkAttendance = ({
  month,
  year,
  location,
  setLocation,
  ...
  const { employees, loading: empLoading } = useSelector((state) => state.superadminEmployees);
  const { locations, loading: locationsLoading } = useSelector((state) => state.superAdminLocations);
  const { loading, attendance, pagination } = useSelector((state) => state.superAdminAttendance);
  ...
  const getOCLeaves = (employee, month, year) => {
    ...
    const openingLeaves = (monthlyLeave.allocated || paidLeavesPerMonth) + (monthlyLeave.carriedForward || 0);
  ...
  useEffect(() => {
    const fetchAllEmployees = async () => {
      try {
        ...
        while (hasMore) {
          const result = await dispatch(fetchEmployees({ location, month, year, page, limit: 10 })).unwrap();
        ...
        if (nextMonthDate <= currentDate) {
          ...
          hasMore = true;
          while (hasMore) {
            const result = await dispatch(fetchEmployees({ location, month: nextMonth, year: nextYear, page, limit: 10 })).unwrap();
            allEmployees = [...allEmployees, ...result.employees];
            hasMore = page < result.pagination.totalPages;
            page += 1;
          }
        ...
        allEmployees.forEach((emp) => {
          if (employeeMap.has(emp._id)) {
            ...
            const mergedLeaves = [...new Map([...(existing.monthlyLeaves || []), ...(emp.monthlyLeaves || [])].map((ml) => [`${ml.year}-${ml.month}`, ml])).values()];
    ...
    if (!user || user.role !== 'super_admin') {
      toast.error('Unauthorized access. Please log in as a superadmin.', { duration: 5000 });
      navigate('/login');
      return;
    }
    dispatch(fetchLocations());
    fetchAllEmployees(); // Superadmin fetches all employees regardless of location
  }, [dispatch, user, navigate, location, month, year]);

  useEffect(() => {
    if (!selectedDate) return;
    const dateStr = format(selectedDate, 'yyyy-MM-dd');
    dispatch(fetchAttendance({ date: dateStr, location, month, year, page: currentPage, limit: pageSize }));
  }, [dispatch, selectedDate, location, month, year, currentPage]);

  const displayData = useMemo(() => {
    if (!selectedDate) {
      return { records: [], totalPages: 1, totalItems: 0, filteredEmployees: [] };
    }
    ...
    const attendanceMap = new Map(
      Array.isArray(attendance)
        ? attendance.filter((record) => record.date && record.date.startsWith(dateStr)).map((record) => [record.employee?._id?.toString(), record])
        : []
    );
    ...
    const filteredEmployees = Array.isArray(employees)
      ? employees.filter((emp) => {
          const matchesFilter = employeeFilter
            ...
            : true;
          return matchesFilter; // No location restriction for superadmin
        })
      : [];
      ...
      records: paginatedEmployees.map((emp) => {
        ...
        return {
          employee: {
            ...
            paidLeaves: emp.paidLeaves || { available: 0, used: 0, carriedForward: 0 },
          },
  ...
  }, [employees, attendance, employeeFilter, selectedDate, location, currentPage, pageSize]);
  ...
  const existingAttendanceRecords = useMemo(() => {
    ...
    const dateStr = format(selectedDate, 'yyyy-MM-dd');
    return attendance.filter((record) => record.date && record.date.startsWith(dateStr));
  ...
  const handleBulkSelect = (employeeId) => {
    setBulkSelectedEmployees((prev) => {
      const newSelection = prev.includes(employeeId) ? prev.filter((id) => id !== employeeId) : [...prev, employeeId];
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      const allEmployeeIds = displayData.records.map((record) => record.employee._id?.toString()).filter(Boolean);
      setBulkSelectedEmployees((prev) => [...new Set([...prev, ...allEmployeeIds])]);
      setBulkEmployeeStatuses((prevStatuses) => {
        const newStatuses = { ...prevStatuses };
      ...
      const allEmployeeIds = displayData.records.map((record) => record.employee._id?.toString()).filter(Boolean);
      setBulkSelectedEmployees((prev) => prev.filter((id) => !allEmployeeIds.includes(id)));
      setBulkEmployeeStatuses((prevStatuses) => {
        const newStatuses = { ...prevStatuses };
        allEmployeeIds.forEach((id) => {
  ...
  const handleBulkSubmit = (preview = false) => {
    ...
    const remainingEmployees = displayData.filteredEmployees.filter((emp) => !bulkSelectedEmployees.includes(emp._id?.toString()));
    ...
    if (existingAttendanceRecords.length > 0) {
      toast.error(`Attendance already marked for selected employees on ${format(selectedDate, 'PPP')}.`, { duration: 5000 });
      return;
    }
  ...
  const confirmBulkSubmit = () => {
    ...
    if (!allRecords.length) {
      ...
      setBulkConfirmDialog({ open: false, records: [], remaining: [], preview: false });
      return;
    }
    ...
    dispatch(bulkMarkAttendance({ attendance: allRecords }))
      ...
      .then(async (response) => {
        ...
        const results = await Promise.all([
          dispatch(fetchEmployees({ location, month, year, page: 1, limit: 10 })).unwrap(),
          dispatch(fetchEmployees({ location, month: nextMonth, year: nextYear, page: 1, limit: 10 })).unwrap(),
          dispatch(fetchEmployees({ location, month: nextNextMonth, year: nextNextYear, page: 1, limit: 10 })).unwrap(),
        ]);
      ...
      .catch((err) => {
        ...
        if (errorMessage === 'Attendance array is required and must not be empty') {
          userFriendlyMessage = 'No valid attendance records to submit.';
        } else if (errorMessage.includes('already marked') || errorMessage.includes('E11000 duplicate key')) {
    ...
    <div className="space-y-6 p-4 animate-fade-in">
      ...
      <Card className="bg-body text-body border border-complementary rounded-lg shadow-md">
        <CardHeader className="border-b border-complementary">
          <CardTitle className="text-2xl font-bold text-body">Mark Bulk Attendance</CardTitle>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label htmlFor="month" className="text-sm font-semibold text-body">Month</Label>
              <Select value={month.toString()} onValueChange={handleMonthChange}>
                <SelectTrigger id="month" className="bg-body text-body border-complementary hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent h-10 rounded-md">
                ...
                <SelectContent className="bg-body text-body border-complementary">
                  {Array.from({ length: 12 }, (_, i) => ({
                    ...
                    <SelectItem key={obj.value} value={obj.value.toString()} className="text-sm hover:bg-accent-light">
                      {obj.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="year" className="text-sm font-semibold text-body">Year</Label>
              <Select
                value={year.toString()}
                onValueChange={(val) => {
                  ...
                  setYear(newYear);
                  if (selectedDate) {
                    const newDate = startOfMonth(new Date(newYear, month - 1, 1));
                    if (newDate <= new Date()) {
                      setSelectedDate(newDate);
                    } else {
                      setSelectedDate(null);
              ...
              >
                <SelectTrigger id="year" className="bg-body text-body border-complementary hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent h-10 rounded-md">
                  <SelectValue placeholder="Select year" />
                </SelectTrigger>
                <SelectContent className="bg-body text-body border-complementary">
                  {[2024, 2025, 2026].map((y) => (
                    <SelectItem key={y} value={y.toString()} className="text-sm hover:bg-accent-light">{y}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="location" className="text-sm font-semibold text-body">Location</Label>
              <Select value={location} onValueChange={setLocation}>
                <SelectTrigger id="location" className="bg-body text-body border-complementary hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent h-10 rounded-md">
                  <SelectValue placeholder="Select location" />
                </SelectTrigger>
                <SelectContent className="bg-body text-body border-complementary">
                  <SelectItem value="all" className="text-sm hover:bg-accent-light">All Locations</SelectItem>
                  {locations.map((loc) => (
                    <SelectItem key={loc._id} value={loc._id} className="text-sm hover:bg-accent-light">{loc.name}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="date" className="text-sm font-semibold text-body">Select Date</Label>
              <Popover>
            ...
            <div className="space-y-2">
              <Label htmlFor="time" className="text-sm font-semibold text-body">Current Time</Label>
              <Input
                type="text"
                id="time"
                value={selectedTime}
                ...
                disabled={locationsLoading || !selectedDate}
          ...
          {existingAttendanceRecords.length > 0 && (
            <Alert className="border-error bg-body text-error rounded-lg p-3 animate-pulse">
              <AlertDescription>
                Attendance already marked for {existingAttendanceRecords.length} employee(s) on {format(selectedDate, 'PPP')}.
              </AlertDescription>
            </Alert>
          )}
          <div className="flex flex-col sm:flex-row items-end gap-4">
            <div className="space-y-2 flex-1">
              <Label htmlFor="employeeFilter" className="text-sm font-semibold text-body">Filter Employees</Label>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="text-sm text-complementary underline cursor-help">Detail</span>
                    </TooltipTrigger>
              ...
              <div className="flex items-center gap-3">
                <Button
                  ...
                  disabled={loading || empLoading || locationsLoading || !selectedDate || !selectedTime || !displayData.filteredEmployees.length}
            ...
            {empLoading || locationsLoading ? (
                    ...
                    <TableHeader className="sticky top-0 bg-complementary shadow-sm z-10">
                      <TableRow>
                        <TableHead className="text-body text-sm w-12">
                          <Checkbox
                            checked={bulkSelectedEmployees.length >= displayData.records.length && displayData.records.every((record) => bulkSelectedEmployees.includes(record.employee._id?.toString()))}
                            onCheckedChange={handleSelectAll}
                            disabled={locationsLoading || !displayData.records.length}
                    ...
                    <TableBody>
                      {displayData.records.map((record, index) => (
                        <TableRow
                          ...
                          className={`${index % 2 === 0 ? 'bg-body' : 'bg-complementary-light'} animate-slide-in-row`}
                          style={{ animationDelay: `${index * 0.05}s` }}
                        >
                          <TableCell className="whitespace-nowrap">
                            <Checkbox
                              checked={bulkSelectedEmployees.includes(record.employee._id?.toString())}
                              onCheckedChange={() => handleBulkSelect(record.employee._id?.toString())}
                              disabled={locationsLoading || !record.employee._id}
                              className="h-5 w-5 border-complementary"
                          ...
                          <TableCell className="text-body whitespace-nowrap">
                            <Select
                              value={
                                bulkSelectedEmployees.includes(record.employee._id?.toString())
                                  ? bulkEmployeeStatuses[record.employee._id?.toString()] || 'absent'
                                  : record.status || 'present'
                              }
                              onValueChange={(value) => handleBulkStatusChange(record.employee._id?.toString(), value)}
                              disabled={!bulkSelectedEmployees.includes(record.employee._id?.toString()) || locationsLoading || !record.employee._id}
                            >
                              <SelectTrigger className="w-full sm:w-32 bg-body text-body border-complementary hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent h-10 rounded-md" aria-label={`Select status for ${record.employee.name}`}>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent className="bg-body text-body border-complementary">
                                <SelectItem value="present" className="text-sm hover:bg-accent-light">Present</SelectItem>
                                <SelectItem value="absent" className="text-sm hover:bg-accent-light">Absent</SelectItem>
                                <SelectItem value="half-day" className="text-sm hover:bg-accent-light">Half-Day</SelectItem>
                                <SelectItem value="leave" disabled={monthlyLeavesAvailable[record.employee._id?.toString()] < 1} className="text-sm hover:bg-accent-light">Leave</SelectItem>
                              </SelectContent>
                            </Select>
                          </TableCell>
                          <TableCell className="text-body text-sm whitespace-nowrap">{record.ocLeaves}</TableCell>
                          <TableCell className="text-body text-sm whitespace-nowrap">{record.leavesAvailable}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {getPageNumbers().map((page) => (
                      <Button
                        ...
                        className={`${currentPage === page ? 'bg-accent text-body hover:bg-accent-hover' : 'border-complementary text-body hover:bg-complementary-light'} text-sm w-10 h-10 rounded-md`}
              ...
              <p className="text-body text-sm text-center py-4">
                {employees.length > 0 ? 'No employees match the current filter.' : 'No employees found.'}
              </p>
            )}
          </div>
        </CardContent>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setBulkConfirmDialog({ open: false, records: [], remaining: [], preview: false })}
      >
        <DialogContent className="bg-body text-body border-complementary max-w-[90vw] sm:max-w-2xl animate-scale-in">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-body">{bulkConfirmDialog.preview ? 'Preview Bulk Attendance' : 'Confirm Bulk Attendance'}</DialogTitle>
            <DialogDescription className="text-body">
              {bulkConfirmDialog.preview ? 'Review the attendance records before submitting.' : 'Please confirm the attendance records to be submitted.'}
            </DialogDescription>
          </DialogHeader>
            ...
            <Table className="border border-complementary">
              ...
              <TableBody>
                {bulkConfirmDialog.records.map((record, index) => {
                  const employee = employees.find((emp) => emp._id?.toString() === record.employeeId);
                  return (
                    <TableRow
                      ...
                      style={{ animationDelay: `${index * 0.05}s` }}
                    >
                      <TableCell className="text-body text-sm">{employee ? `${employee.name} (${employee.employeeId})` : 'Unknown'}</TableCell>
                      <TableCell className="text-body text-sm capitalize flex items-center gap-2">{getStatusIcon(record.status)} {record.status}</TableCell>
                      <TableCell className="text-body text-sm">{format(toZonedTime(new Date(record.date), 'Asia/Kolkata'), 'PPP hh:mm:ss a')}</TableCell>
                    </TableRow>
                  );
                })}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setBulkConfirmDialog({ open: false, records: [], remaining: [], preview: false })}
