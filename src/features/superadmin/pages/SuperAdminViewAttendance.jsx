...
const SuperAdminViewAttendance = () => {
  ...
  const { locations, loading: locLoading } = useSelector(
    (state) => state.superAdminLocations
  );
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text("Superadmin Attendance Records Report", 15, 15);
    doc.setFontSize(10);
    doc.text(`Location: ${locations.find((loc) => loc._id === locationId)?.name || "All Locations"}`, 15, 22);
    ...
    doc.save(`Superadmin_Attendance_Records_${format(new Date(), "yyyy-MM-dd")}.pdf`);
  ...
  const handleDownloadExcel = () => {
    ...
    const headerWs = XLSX.utils.json_to_sheet([
      { A: 'Superadmin Attendance Records Report' },
      { A: `Location: ${locations.find((loc) => loc._id === locationId)?.name || "All Locations"}` },
    ...
    const filename = `Superadmin_Attendance_Records_${format(new Date(), "yyyy-MM-dd")}.xlsx`;
        ...
        <CardHeader className="border-b border-complementary flex flex-row justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              disabled={attLoading || empLoading || locLoading || !attendance.length}
