...
const SuperAdminUserManagement = () => {
  ...
  const handleUpdateUser = async (data) => {
    setLoading(true);
    try {
      const response = await api.put(`/superadmin/users/${editingUserId}`, data);
  ...
  const handleEditUser = (user) => {
    ...
    form.setValue('locations', user.locations.map(loc => loc._id.toString()));
  };
    ...
    <Layout title="user-management">
      <div className={cn(
        ...
        <Card className={cn(
          ...
          <CardHeader>
            <CardTitle className="text-2xl font-bold">User Management (Super Admin)</CardTitle>
          </CardHeader>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleUpdateUser)} className="space-y-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Role <span className="text-error">*</span></FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Locations (Site Incharge only)</FormLabel>
                        <FormControl>
                          <Select
                            value={field.value}
                            onValueChange={field.onChange}
                            multiple
                            disabled={form.watch('role') === 'admin' || loading}
                          >
                            <SelectTrigger className="w-full bg-complementary text-body border-accent h-10">
                  ...
                  <div className="flex space-x-2">
                    <Button
                      type="submit"
                      className={cn(
                        'w-full bg-accent text-white hover:bg-accent-hover h-10',
                        loading && 'opacity-75'
                      )}
                      disabled={loading}
                    >
                    ...
                    <Button
                      ...
                      className={cn(
                        'w-full bg-gray-500 text-white hover:bg-gray-600 h-10',
                        loading && 'opacity-75'
                      )}
                      disabled={loading}
                    >
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p><strong>Locations:</strong> {user.locations.map(loc => loc.name).join(', ') || 'None'}</p>
                        </div>
