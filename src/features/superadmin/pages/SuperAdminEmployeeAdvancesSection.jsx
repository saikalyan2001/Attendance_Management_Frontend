...
import { fetchEmployeeAdvances } from '../redux/superadminEmployeeSlice';

const SuperAdminEmployeeAdvancesSection = ({
  id,
  employeeName,
  currentPage,
  setCurrentPage,
  ...
  // Use custom equality function to stabilize useSelector
  const { advances, advancesPagination, isLoading, error } = useSelector(
    (state) => state.superadminEmployees,
    (prev, next) =>
      prev.advances === next.advances &&
      prev.advancesPagination?.currentPage === next.advancesPagination?.currentPage &&
      prev.advancesPagination?.itemsPerPage === next.advancesPagination?.itemsPerPage &&
      prev.advancesPagination?.sortField === next.advancesPagination?.sortField &&
      prev.advancesPagination?.sortOrder === next.advancesPagination?.sortOrder &&
      prev.advancesPagination?.totalItems === next.advancesPagination?.totalItems &&
      prev.advancesPagination?.totalPages === next.advancesPagination?.totalPages &&
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= advancesPagination?.totalPages) {
      setCurrentPage(page);
      dispatch(
        fetchEmployeeAdvances({ id, page, limit: itemsPerPage, sortField, sortOrder })
      )
        .unwrap()
        .catch((error) => {
          const parsedError = parseServerError(error);
            ...
            {advancesPagination?.totalPages > 1 && !isLoading && (
              <div className="flex justify-between items-center mt-4 xs:mt-5 sm:mt-6">
                ...
                <div className="flex flex-wrap justify-center items-center gap-2">
                  {Array.from({ length: advancesPagination?.totalPages || 1 }, (_, i) => i + 1).map((page) => (
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
                        onClick={() => handlePageChange(currentPage + 1)}
                        disabled={currentPage === advancesPagination?.totalPages || isLoading}
                        className="border-accent text-accent hover:bg-accent-hover hover:text-body rounded-lg px-2 xs:px-3 sm:px-4 py-1 xs:py-2 min-h-[36px] text-xs xs:text-sm sm:text-base transition-all duration-300 focus:ring-2 focus:ring-accent focus:ring-offset-2"
...
export default SuperAdminEmployeeAdvancesSection;