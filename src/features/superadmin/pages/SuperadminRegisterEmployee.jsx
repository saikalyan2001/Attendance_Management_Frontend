...
const SuperadminRegisterEmployee = () => {
  ...
  const [lastError, setLastError] = useState(null); // Track last error to prevent duplicate toasts
  const maxRetries = 3;
  ...
  const handleSubmit = useCallback(
    async (data) => {
      try {
        ...
        // Clear previous error state and toasts before submission
        ...
        // Toast moved to useEffect to avoid duplicates
  ...
  useEffect(() => {
    if (employeesError && errorType === "single" && employeesError !== lastError) {
      ...
      setServerError(parsedError);
      setLastError(employeesError); // Track this error to prevent duplicate toasts
      ...
      // Reset error state after displaying toasts to prevent re-triggering
      setTimeout(() => {
        dispatch(resetEmployees());
        setLastError(null);
      }, 100);
    }
  }, [employeesError, errorType, dispatch, handleSubmit, lastError, retryCount]);
