...
const Settings = () => {
  ...
  const { settings, loadingFetch, loadingUpdate, loadingLeaves, error, successUpdate, successLeaves } = useSelector((state) => state.superAdminSettings);
  ...
  const handleUpdateClick = async () => {
    try {
      ...
      if (form.getValues('updatePaidLeavesPerYear') && form.getValues('applyLeaveChanges')) {
        ...
        const response = await fetch('http://localhost:5000/api/superadmin/employees/count', {
          headers: {
