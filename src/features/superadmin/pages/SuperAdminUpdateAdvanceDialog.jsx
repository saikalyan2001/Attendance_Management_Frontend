...
import { updateEmployeeAdvance, fetchEmployeeAdvances } from '../redux/superadminEmployeeSlice';
...
const SuperAdminUpdateAdvanceDialog = ({ open, onOpenChange, employee, setSuccessMessage }) => {
 ...
 const handleSubmit = async (data) => {
  try {
    ...
    console.log('Submitting advance update with payload:', { id: employee._id, ...payload });

    await dispatch(updateEmployeeAdvance({ id: employee._id, ...payload })).unwrap();
    await dispatch(fetchEmployeeAdvances({ id: employee._id, page: 1, limit: 5, sortField: 'year', sortOrder: 'desc' })).unwrap();
    setSuccessMessage('Employee advance updated successfully'); // Only set the success message
    onOpenChange(false);
    form.reset();
  } catch (error) {
